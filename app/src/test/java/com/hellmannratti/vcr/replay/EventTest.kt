...
class EventTest {
    ...
    @Test fun `serialize and deserialize SessionStartEvent`() {
        val e: Event = SessionStartEvent(1L, appVersion = "1.0.0", device = "emulator")
    ...
    @Test fun `serialize and deserialize ActionEvent`() {
        val e: Event = ActionEvent(2L, name = "Click", details = buildJsonObject { put("x", 10); put("y", 20) })
        val s = json.encodeToString(Event.serializer(), e)
        val back = json.decodeFromString<Event>(s)
    ...
    @Test fun `serialize and deserialize Request and Response events`() {
        val req: Event = RequestEvent(3L, requestId = "abc", method = "GET", url = "https://example.com", bodySha256 = null)
        val res: Event = ResponseEvent(4L, requestId = "abc", code = 200, headers = mapOf("content-type" to "text/plain"), body = "ok", durationMs = 5)
