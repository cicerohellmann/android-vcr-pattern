...
class UrlPattern private constructor(
    ...
    /**
     * Extract parameter values from a URL that matches this pattern.
     * Returns a map of placeholder names to their values.
     */
    fun extract(url: String): Map<String, String> {
        ...
        if (urlSegments.size != patternSegments.size) return emptyMap()
        
        for (i in segments.indices) {
            when (val seg = segments[i]) {
                is Segment.Placeholder -> {
                    if (i < urlSegments.size) {
                        result[seg.name] = urlSegments[i]
                    }
                }
                is Segment.Literal -> {
                    // Skip literals
